...
CREATE TABLE watches_schema.watch  (
  watch_code varchar(255) UNIQUE PRIMARY KEY,
  watch_reference varchar(255) UNIQUE,
  brand varchar(255),
...
ALTER TABLE prices ADD FOREIGN KEY (watch_reference) REFERENCES watch (watch_reference);
...
ALTER TABLE characteristics ADD FOREIGN KEY (watch_id) REFERENCES prices (watch_id);

ALTER TABLE seller ADD FOREIGN KEY (seller_id) REFERENCES prices (seller_id);

ALTER TABLE financial_information ADD FOREIGN KEY (holding_company) REFERENCES board (holding_company);
