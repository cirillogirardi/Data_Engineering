...
def swiss_editing():
    ...
    AP.to_parquet(Local_Dir + 'watches.parquet')
...
def find_social_info():
    ...
    social_info_df.to_parquet(Local_Dir + 'social_info.parquet')
...
def find_financial_info():
    ...
    company_financial_df.to_parquet(Local_Dir + 'financial_info.parquet')
...
def company_editing():
    ...
    df = df.join(desc)
    df.drop(columns=["Unnamed: 0", 'Key people', 'Revenue', 'Parent', 'Website'], inplace=True)
    ...
    df.to_parquet(Local_Dir + 'more_information.parquet')
...
def chrono_editing():
    ...
    df4.to_parquet(Local_Dir + 'last.parquet')
...
with DAG(
    ...
    upload_S3_finalized_watches = PythonOperator(
        ...
        op_kwargs={
                'filename': Local_Dir + 'last.parquet',
                'key': 'last.parquet',
    ...
    upload_S3_financial_info = PythonOperator(
        ...
        python_callable=upload_S3,
        op_kwargs={
                'filename': Local_Dir + 'financial_info.parquet',
                'key': 'financial_info.parquet',
                'bucket_name': 'cartier-bucket'
        }
    )
    upload_S3_company_information = PythonOperator(
        ...
        python_callable=upload_S3,
        op_kwargs={
                'filename': Local_Dir + 'more_information.parquet',
                'key': 'more_information.parquet',
                'bucket_name': 'cartier-bucket'
        }
    )
    ...
    upload_S3_social_info = PythonOperator(
        ...
        python_callable=upload_S3,
        op_kwargs={
            'filename': Local_Dir + 'social_info.parquet',
            'key': 'social_info.parquet',
            'bucket_name': 'cartier-bucket'
        }
    )
    ...
    upload_S3_swiss_watches = PythonOperator(
        ...
        python_callable=upload_S3,
        op_kwargs={
            'filename': Local_Dir + 'watches.parquet',
            'key': 'watches.parquet',
            'bucket_name': 'cartier-bucket'
        }
    )
